...
W1. Every cell of `This week` names its own meal, in gold. With nothing picked it reads
    `Pick a meal` in grey inside a dashed border; a meal turned off reads `Hidden` in grey.
    It never reads `Work`, `Home` or `Hide` on its own.
W2. Tap a cell. Nothing opens over the page — no menu, no overlay, no panel. The meal
    turns off (`Hidden`, and its section below disappears) and that cell's day becomes the
    selected day. Tap it again and it comes back, still holding whatever was picked.
W3. With a day selected, pick the meal itself in its section below — tick foods in Morning
    and Afternoon, tap a dinner in Night. The cell names what you picked as you go: ticked
    foods comma-separated, the dinner by name, `♻️ <dinner>` for a leftovers card, and
    `🍽️ Ordering out` / `✅ Already full` for those.
W4. `Hide this meal`, beside `Edit list` in a section, does the same as tapping its cell.
W5. Tapping a day's name selects that day without changing any cell.
W6. The toolbar under the grid counts the selected day: `2 of 3 meals picked on Monday`,
    with `, 1 hidden` after it when something is.
W7. Reload and confirm every cell comes back as left — picked, empty or hidden.
